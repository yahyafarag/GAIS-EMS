...
const FieldEditorItem = ({ field, index, total, onMove, onDelete, onUpdate }: { 
    field: DynamicField, 
    index: number,
    total: number,
    onMove: (dir: -1 | 1) => void,
    ...
    return (
        ...
        >
            <GlassCard className="p-4 border-l-4 border-l-indigo-500 hover:border-indigo-400 transition-colors">
                        ...
                        <div className="md:col-span-3">
                            ...
                            <div className="relative">
                                <select 
                                    ...
                                    className="w-full bg-slate-900 border border-white/10 rounded-lg px-3 py-2 text-slate-300 outline-none appearance-none"
                                >
...
export const SystemDesigner: React.FC = () => {
  const { config, addField, removeField, updateField, updateFieldOrder } = useApp();
  ...
  const handleMove = async (index: number, direction: -1 | 1) => {
      if (index + direction < 0 || index + direction >= displayFields.length) return;
      await updateFieldOrder(currentSection, index, index + direction);
  };
                ...
                >
                    <div className="space-y-4">
                        ...
                        <AnimatePresence initial={false}>
                            {displayFields.map((field, index) => (
                                <FieldEditorItem 
                                    ...
                                    onUpdate={(updates) => updateField(currentSection, field.id, updates)}
                                    onDelete={() => {
                        ...
                        <motion.button
                            ...
                            className="w-full py-6 rounded-2xl border-2 border-dashed border-indigo-500/30 hover:border-indigo-500 hover:bg-indigo-500/10 transition-all flex items-center justify-center gap-2 text-indigo-400 hover:text-indigo-300 group"
                        >
